---
title: Dragons Edge
permalink: /server
---

![Server Banner](assets/server_banner.png)

There is an official Age of Berk server. All are welcome to join. For now, only those who are authenticated with Microsoft will be able to log in.

By joining, you agree to the following rules:
1. No griefing, targeting, bullying, or unkindness of any kind.
2. No swearing, slurs, or vulgarities of any kind. This includes, but is not limited to, written books, signs, and builds.
3. Anyone known to have broken these rules will be banned. Just please be kind. Access to this server is a privilege, not a right

Server Address:
![Server Address](assets/server.png)
